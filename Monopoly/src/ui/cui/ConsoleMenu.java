...
public class ConsoleMenu implements Menu, Serializable {
	...
	private void playerPropertiesMenu(Player player) {
		...
		menuOptions += "Waehlen Sie ein Grundstueck um die anderen Grundstuecke der Gruppe zu sehen.\n";
		...
		for(int i = 0; i < options.length; i++) {
			switch(options[i]) {
				...
				case 21:
					...
					break;
				case 30:
					menuOptions += "[" + (i + 1) + "] 1000 DM zahlen um aus dem Gefaengnis frei zu kommen.\n";
					break;
				case 31:
		...
		for(int i = 0; i < options.length; i++) {
			switch(options[i]) {
				...
				case 3:
					...
					break;
				case 22:
					menuOptions += "[" + (i + 1) + "] Hypothek aufnehmen.\n";
					break;
				case 23:
					menuOptions += "[" + (i + 1) + "] Hypothek abzahlen.\n";
					break;
				case 13:
	...
	private PurchasableCircularList showMortageMenu(Player player, boolean take) {
		...
		ArrayList<PurchasableCircularList> property = player.getProperties();
		if(take) {
			System.out.println("Hypothek aufnehmen:");
		} else {
			System.out.println("Hypothek abbezahlen:");
		}
