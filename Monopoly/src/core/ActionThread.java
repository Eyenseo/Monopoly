...
public class ActionThread extends Thread {
	 ...
	 * FieldCircularList, Thread, HashMap) Thread} {@code (null, null, gname)}, where {@code gname} is a newly
	 * generated
	 ...
	 * <b><i>Taken of the official JavaDoc</i></b> Allocates a new {@code Thread} object. This constructor has the same
	 * effect as {@linkplain #ActionThread(ThreadGroup, Runnable, String, HashMap, ActionData, ServerOperator,
	 * FieldCircularList, Thread, HashMap) Thread} {@code (null, target, gname)},
	 * where {@code gname} is a newly generated
	 * name. Automatically generated names are of the form {@code "Thread-"+}<i>n</i>, where <i>n</i> is an integer.
	 *
	...
	public ActionThread(ThreadGroup group, Runnable target, HashMap<Integer, Player> playerHashMap,
	                    ActionData actionData, ServerOperator serverOperator, FieldCircularList jail,
	                    Thread turnThread,
	                    HashMap<Integer, HashMap<String, ActionThread>> actions) {
		super(group, target);
		this.playerHashMap = playerHashMap;
	...
	/**
	 * <b><i>Taken of the official JavaDoc</i></b> Allocates a new {@code Thread} object so that it has {@code
	 * target} as
	 * its run object, has the specified {@code name} as its name, and belongs to the thread group referred to by
	 * {@code
	 * group}.
	 * <p/>
	 ...
	 * currently running thread. The method {@linkplain #setPriority setPriority} may be used to change the priority
	 * to a
	 * new value.
	 * <p/>
	 * <p>The newly created thread is initially marked as being a daemon thread if and only if the thread creating
	 * it is
	 * currently marked as a daemon thread. The method {@linkplain #setDaemon setDaemon} may be used to change
	 * whether or
	 ...
	 * ServerOperator, FieldCircularList, Thread, HashMap)}with the exception of the fact that it allows the thread
	 * stack
	 * size to be specified.  The stack size is the approximate number of bytes of address space that the virtual
	 * machine
	 * is to allocate for this thread's stack.  <b>The effect of the {@code stackSize} parameter, if any, is highly
	 * platform dependent.</b>
	 * <p/>
	 * <p>On some platforms, specifying a higher value for the {@code stackSize} parameter may allow a thread to
	 * achieve
	 * greater recursion depth before throwing a {@link StackOverflowError}. Similarly,
	 * specifying a lower value may allow
	 * a greater number of threads to exist concurrently without throwing an {@link OutOfMemoryError} (or other
	 * internal
	 ...
	 * <p/>
	 * <p>The virtual machine is free to treat the {@code stackSize} parameter as a suggestion.  If the specified
	 * value is
	 * unreasonably low for the platform, the virtual machine may instead use some platform-specific minimum value; if
	 * the
	 * specified value is unreasonably high, the virtual machine may instead use some platform-specific maximum.
	 * Likewise,
	 ...
	 * <p/>
	 * <p><i>Due to the platform-dependent nature of the behavior of this constructor,
	 ...
	 * implementation
	 * to another.  In light of this variation, careful tuning of the stack size parameter may be required,
	 * and the tuning
	 * may need to be repeated for each JRE implementation on which an application is to run.</i>
	 * <p/>
	 * <p>Implementation note: Java platform implementers are encouraged to document their implementation's behavior
	 * with
	 ...
	 * @param target    the object whose {@code run} method is invoked when this thread is started. If {@code null},
	 *                     this
	 *                  thread's run method is invoked.
	 * @param name      the name of the new thread
